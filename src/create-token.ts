...
async function getKeypairFromPhantom(): Promise<Keypair> {
...
async function createVCoinToken() {
  ...
  if (authorityBalance < 0.05 * LAMPORTS_PER_SOL) {
    ...
    console.error('Please fund your wallet with at least 0.05 SOL on devnet before continuing.');
    process.exit(1);
  }
  ...
  try {
    ...
    console.error('\nPlease ensure:');
    console.error('1. Your wallet has enough SOL (at least 0.05 SOL)');
    console.error('2. You are connected to the correct network');
    console.error('3. The private key you entered is correct');
    process.exit(1);
  }
}
